...
<style>
    .notif-container { flex: 1; padding: 24px; max-width: 800px; margin: 0 auto; }
    .notif-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; background: #fff; padding: 20px 24px; border-radius: 12px; border: 1px solid var(--border-light); }
    .notif-card { display: flex; align-items: center; gap: 16px; padding: 16px 20px; background: #fff; border: 1px solid var(--border-light); border-bottom: none; cursor: pointer; transition: 0.2s; text-decoration: none; color: inherit; }
    .notif-card:first-of-type { border-radius: 12px 12px 0 0; }
    .notif-card:last-of-type { border-bottom: 1px solid var(--border-light); border-radius: 0 0 12px 12px; }
    .notif-card:hover { background: #f8fafc; }
    .notif-card.unread { background: #F0F9FF; }
    .notif-card.unread:hover { background: #E0F2FE; }
    .unread-dot { width: 10px; height: 10px; background: #3B82F6; border-radius: 50%; flex-shrink: 0; }
    .notif-icon-wrapper { position: relative; }
    .notif-badge { position: absolute; bottom: -2px; right: -2px; width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid #fff; color: white; }
    .badge-like { background: #EF4444; }
...
<script>
    ...
    function markAllRead() {
        ...
        fetch('notifications.php', { method: 'POST', body: fd }).then(() => {
